...
parser.add_argument('-f', '--filter', help='filter (e.g., ":,:", ":,2:")')
...
parser.add_argument('-s', '--save', help='image save', action='store_true', default=False)
...
parser.add_argument('-r', '--remove-outlier', help='remove outliers', action='store_true', default=False)
...
if not args.nosummary:
    ...
    if args.byrow:
        ...
        print '%7s' % 'SEQ', ' '.join(map(lambda x: '%7s' % ('#'+str(x)), range(nsteps))), \
            '%7s' % 'AVG', '%7s' % 'STD', '%7s' % 'Q1', '%7s' % 'Q2', '%7s' % 'Q3'
        print '%7s' % 'Time', ' '.join(map(lambda x: '%7.3f'%x, np.nanmean(elap[:,::npe], 1))), \
            '%7.3f' % np.nanmean(elap[:,::npe]), '%7.3f' % np.nanstd(elap[:,::npe], ddof=1), \
            '%7.3f' % np.nanpercentile(elap[:,::npe], 25), \
            '%7.3f' % np.nanpercentile(elap[:,::npe], 50), \
            '%7.3f' % np.nanpercentile(elap[:,::npe], 75)
        print '%7s' % 'AVG', ' '.join(map(lambda x: '%7.3f'%x, np.nanmean(elap, 1))), \
            '%7.3f' % np.nanmean(elap), '%7.3f' % np.nanstd(elap, ddof=1), \
            '%7.3f' % np.nanpercentile(elap, 25), \
            '%7.3f' % np.nanpercentile(elap, 50), \
            '%7.3f' % np.nanpercentile(elap, 75)
